...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (backup) {
    ...
    // Só mostra botão de restaurar se houver itens pendentes
...
// ==========================================
// 1. VALIDAÇÃO MATEMÁTICA (O SEGREDO DA PERFORMANCE)
// ==========================================
function validarMatematicaCNPJ(cnpj) {
  ...
  return true; // CNPJ matematicamente válido!
}
...
window.processarEntrada = () => {
  ...
  if (document.getElementById('tabArquivo').style.display !== 'none' && inputArquivo.files.length > 0) {
    ...
    reader.onload = (e) => {
      iniciarComLista(e.target.result);
    };
...
function iniciarComLista(textoBruto) {
  ...
  indiceAtual = 0;
  errosSeguidos = 0;
  limparSessao(); // Limpa backup antigo
...
async function rodarProcesso() {
  ...
  document.getElementById('exportArea').style.display = 'flex'; // Permite exportar parcial
  ...
  for (let i = indiceAtual; i < listaCNPJs.length; i++) {
    ...
    if (!validarMatematicaCNPJ(cnpjLimpo)) {
      ...
      // Se a matemática bater, aí sim consulta a API
    ...
    // Atualizações Finais do Ciclo
    indiceAtual = i + 1;
    elTotal.innerText = `${indiceAtual} / ${listaCNPJs.length}`;
    document.getElementById('progressBar').value = indiceAtual;

    // Salva backup a cada 10 itens (segurança)
    if (i % 10 === 0) salvarProgresso();

    // Delay inteligente: Se foi validado localmente (erro), é rápido. Se foi API, espera.
  ...
  if (indiceAtual >= listaCNPJs.length) {
    ...
    limparSessao(); // Limpa backup pois terminou
  }
}
...
function ativarModoPausa() {
  ...
  salvarProgresso(); // Garante salvamento na pausa
}
